...
public class MachineService implements IDao<Machine> {
    ...
    public List<Static> Machinesbysalle() {
        List<Static> machines = new LinkedList<>();
        // 1. build the query
        String query = "SELECT  s.libelle ,count(*) as nb FROM " + TABLE_MACHINE+" m inner join "+TABLE_SALLE+" s on s. idS=m.idSalle group by m.idSalle ";
        // 2. get reference to writable DB
        SQLiteDatabase db = this.helper.getWritableDatabase();
        Cursor cursor = db.rawQuery(query, null);
        // 3. go over each row, build Machine and add it to list
        ...
        if (cursor.moveToFirst()) {
            do {
                machine = new Static();
                machine.setNb(Integer.parseInt(cursor.getString(1)));
                machine.setLibelle(cursor.getString(0));
                // Add salle to salle
                machines.add(machine);
            } while (cursor.moveToNext());
        }
